...
Specify the user for whom the service is required.
Enter requester information.
Select the Type of Connection.
Provide existing connection information when applicable.
Enter the Total Amount.
Select the Mode of Payment, such as UPI or Card.
...
Review the basic Catalog Item configuration.
Review the configured Catalog Variables.
Review the Requester Information Variable Set.
Review the Catalog UI Policy.
Click Try It to open the Network Services Request form.
Enter the requester information.
Select the Type of Connection.
Demonstrate the dynamic behavior for an Existing Connection.
Enter the Total Amount.
Select the Mode of Payment.
Enter the required Address.
Submit the request using Order Now.
Review the generated Request record.
Review the Request number, requester, approval status, and request state.
Open the associated Requested Item.
Review the variables entered through the Catalog form.
Review the generated Catalog Tasks.
Verify the Field Services task.
Verify the Software task.
Demonstrate the automated backend process using Flow Designer.
👩‍💻 My Contribution
